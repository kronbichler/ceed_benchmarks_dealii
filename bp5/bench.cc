...
main(int argc, char **argv)
{
  ...
  if (degree == 1)
    ...
    do_test<3, 12, 13>(s, compact_output, comm_shmem);
  else if (degree == 13)
    do_test<3, 13, 14>(s, compact_output, comm_shmem);
  else if (degree == 14)
    do_test<3, 14, 15>(s, compact_output, comm_shmem);
  else if (degree == 15)
    do_test<3, 15, 16>(s, compact_output, comm_shmem);
  else
